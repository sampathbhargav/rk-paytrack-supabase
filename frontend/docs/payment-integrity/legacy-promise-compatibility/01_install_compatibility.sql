-- REVISED V2: preserves original promise values in the existing request ledger.
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('rk_payment_private.promise_state(uuid,date)')
    AND NOT p.prosecdef AND md5(regexp_replace(p.prosrc,'\s','','g'))='26e28483cc7b22b48ca37e286d62021f') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('public.rk_payment_operation(uuid,text,jsonb)')
    AND NOT p.prosecdef AND md5(regexp_replace(p.prosrc,'\s','','g'))='50835a1411fa78d1a300ad60ac228666') THEN
    RAISE EXCEPTION 'Function version mismatch: public.rk_payment_operation(uuid,text,jsonb); nothing changed';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_proc p WHERE p.oid=to_regprocedure('rk_payment_private.schedule(uuid)')
    AND NOT p.prosecdef AND md5(regexp_replace(p.prosrc,'\s','','g'))='17223c9120f997fcd1189425aa6de32a') THEN
    RAISE EXCEPTION 'Function version mismatch: rk_payment_private.schedule(uuid); nothing changed';
  END IF;
END;
$version_guard$;
DO $helper_guard$
BEGIN
  IF to_regprocedure('rk_payment_private.promise_state_legacy_v1(uuid,date,boolean)') IS NOT NULL THEN
    RAISE EXCEPTION 'Compatibility helper already exists; check installation status';
  END IF;
END;
$helper_guard$;
CREATE FUNCTION rk_payment_private.promise_state_legacy_v1(p_deal uuid, p_due date, p_check_snapshot boolean)
...
BEGIN
  ...
  END IF;
  IF basis_count<>1 OR basis IS NULL OR basis<=0 OR basis<>round(basis,2)
    OR basis::text IN ('NaN','Infinity','-Infinity') THEN
    RAISE EXCEPTION 'Historical promise basis needs review';
  END IF;
  SELECT EXISTS (SELECT 1 FROM public.payment_promises p
    WHERE p.deal_id=p_deal AND p.original_due_date=p_due
      AND p.amount_due IS DISTINCT FROM basis) INTO legacy_basis;
  -- Residual-era snapshots are eligible only when their own arithmetic is valid.
  -- This does not reconstruct their creation dates or reallocate any payments.
  IF legacy_basis AND EXISTS (
    SELECT 1 FROM public.payment_promises p
    WHERE p.deal_id=p_deal AND p.original_due_date=p_due AND (
      p.amount_due IS NULL OR p.amount_paid_now IS NULL OR p.remaining_amount IS NULL
      OR p.amount_due<=0 OR p.amount_due>basis OR p.amount_paid_now<0 OR p.remaining_amount<0
      OR p.amount_due::text IN ('NaN','Infinity','-Infinity')
      OR p.amount_paid_now::text IN ('NaN','Infinity','-Infinity')
      OR p.remaining_amount::text IN ('NaN','Infinity','-Infinity')
      OR p.amount_due<>round(p.amount_due,2) OR p.amount_paid_now<>round(p.amount_paid_now,2)
      OR p.remaining_amount<>round(p.remaining_amount,2)
      OR p.amount_due IS DISTINCT FROM p.amount_paid_now+p.remaining_amount
      OR p.promise_status IS NULL
      OR p.promise_status NOT IN ('Pending','Broken','Paid','Partial Paid','Rescheduled','Cancelled')
      OR (p.promise_status='Paid' AND p.remaining_amount<>0)
    )
  ) THEN RAISE EXCEPTION 'Legacy promise arithmetic needs review'; END IF;
  ...
  IF EXISTS(SELECT 1 FROM public.payments x WHERE x.deal_id=p_deal AND x.due_date=p_due
      AND (x.amount_paid IS NULL OR x.amount_paid<0 OR x.amount_paid<>round(x.amount_paid,2) OR x.amount_paid::text IN ('NaN','Infinity','-Infinity')))
  ...
  -- All entry points call the strict wrapper before writing. Only the RPC's
  -- post-mutation reconciliation calls this read-only helper with false:
  -- the old leaf snapshot is necessarily stale until reconciliation completes.
  IF legacy_basis AND p_check_snapshot IS DISTINCT FROM false
    AND leaf.remaining_amount IS DISTINCT FROM greatest(basis-paid_total,0) THEN
    RAISE EXCEPTION 'Legacy promise does not match installment payments; review required';
  END IF;
  RETURN QUERY SELECT leaf.id,leaf.promise_status,basis,paid_total,greatest(basis-paid_total,0);
END;
$function$;
REVOKE ALL ON FUNCTION rk_payment_private.promise_state_legacy_v1(uuid,date,boolean) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION rk_payment_private.promise_state_legacy_v1(uuid,date,boolean) TO authenticated;
...
AS $function$
BEGIN
  RETURN QUERY SELECT * FROM rk_payment_private.promise_state_legacy_v1(p_deal,p_due,true);
END;
$function$;
...
DECLARE
  ...
  promises_json jsonb;
  promise_snapshots_before jsonb;
BEGIN
  ...
  -- Capture under the existing deal/promise locks, before any mutation.
  -- Persist only affected installments below, with the same actor/request ledger.
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    'id',p.id,'deal_id',p.deal_id,'original_due_date',p.original_due_date,
    'amount_due',p.amount_due,'amount_paid_now',p.amount_paid_now,
    'remaining_amount',p.remaining_amount,'promise_status',p.promise_status,
    'promised_date',p.promised_date,'parent_promise_id',p.parent_promise_id,
    'rescheduled_from_date',p.rescheduled_from_date,'created_at',p.created_at
  ) ORDER BY p.id),'[]'::jsonb) INTO promise_snapshots_before
  FROM public.payment_promises p WHERE p.deal_id=deal_id_value;
  ...
  FOREACH due_value IN ARRAY dates LOOP
    SELECT * INTO state FROM rk_payment_private.promise_state_legacy_v1(deal_id_value,due_value,false);
    IF FOUND AND state.leaf_status<>'Cancelled' THEN
      UPDATE public.payment_promises SET amount_due=state.gross,amount_paid_now=state.paid,remaining_amount=state.remaining,
  ...
  SELECT coalesce(jsonb_agg(s.value ORDER BY s.value->>'id'),'[]'::jsonb)
  INTO promise_snapshots_before
  FROM jsonb_array_elements(promise_snapshots_before) s(value)
  WHERE (s.value->>'original_due_date')::date=ANY(dates);
  result:=jsonb_build_object('promiseSnapshotVersion',1,
    'promiseSnapshotsBefore',promise_snapshots_before,'version',1,'promises',promises_json,'legacyStatusPreserved',preserve_legacy_status,'payments',rows_json,'dealId',deal_id_value,'dealStatus',new_status,
...
$function$;
COMMIT;
SELECT 'Legacy promise compatibility v2 (preserved snapshots) installed' AS result;
