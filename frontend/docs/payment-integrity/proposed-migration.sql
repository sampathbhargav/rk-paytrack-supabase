-- PROPOSAL ONLY: not applied to production. Frontend requires activation.
-- Review investigation.md and test on a disposable/staging PostgreSQL database.
-- This adds a gated transaction API; it never backfills or repairs legacy rows.
-- Activation is a separate approved step. See rollout.md.
BEGIN;
